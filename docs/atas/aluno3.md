# ATA de Acompanhamento - Júlia Persson Mascari

## Registro de Atividades Semanais

| Período | Microfundamento | Estudou? | Atividade Prevista | Atividade Realizada |
|:-------:|:---------------:|:--------:|:------------------:|:-------------------:|
| 09/02 - 15/02 | Gestão de Serviços de TI - Unidade 1 | ✅ | - | - |
| 16/02 - 22/02 | Gestão de Serviços de TI - Unidade 2 | ✅ | - | - |
| 23/02 - 01/03 | Arquitetura de Software Distribuído - Unidade 1 e 2 | ✅ | Criação da seção "Problema" e "Requisitos Funcionais" com o time | Criação da seção "Problema" e "Requisitos Funcionais" com o time |
| 02/03 - 08/03 | Arquitetura de Software Distribuído - Unidade 3 | ✅ | Criação da seção "Catálogo de Serviços" | Criação da seção "Catálogo de Serviços" |
| 09/03 - 15/03 | APIs e Web Services - Unidade 1 | ✅ | Planejamento da API | Planejamento da API |
| 16/03 - 22/03 | APIs e Web Services - Unidade 2 | ✅ | Inicio do desenvolvimento da API de Serviço (CRUD) | Inicio do desenvolvimento da API de Serviço (CRUD) |
| 23/03 - 29/03 | APIs e Web Services - Unidade 2 | ✅ | Validação da API e documentação Swagger | Validação da API e documentação Swagger |
| 30/03 - 05/04 | Recuperação de Informação na Web e Redes Sociais - Unidade 1 | ✅ | Finalização da API | Finalização da API |
| 06/04 - 12/04 | Recuperação de Informação na Web e Redes Sociais - Unidade 2 | ✅ | Testes e documentação | Testes e documentação, incluindo preenchimento de toda página docs/backend-apis |
| 13/04 - 19/04 | Computação Ubíqua e Pervasiva e IoT - Unidade 1 | ✅ | Criação do protótipo de interface front-end via Figma | Criação do protótipo de interface front-end via Figma |
| 20/04 - 26/04 | Computação Ubíqua e Pervasiva e IoT - Unidade 2 | ✅ | Ajustes no protótipo para padronização | Ajustes no protótipo para padronização |
| 27/04 - 03/05 | Desenvolvimento de Aplicações Móveis - Unidade 1 | ✅ | Criação da tela de "Serviço" + Modal | Criação da tela de "Serviço" + Modal |
| 04/05 - 10/05 | Desenvolvimento de Aplicações Móveis - Unidade 2 | ✅ | Criação da tela de "Dashboard" + Documentação + Teste | Criação da tela de "Dashboard" + Documentação + Teste |
| 11/05 - 17/05 | Desenvolvimento de Aplicações Móveis - Unidade 2 | ⬜ |  |  |
| 18/05 - 24/05 | Fundamentos de Projeto e Análise de Algoritmos - Unidade 1 | ⬜ |  |  |
| 25/05 - 31/05 | Fundamentos de Projeto e Análise de Algoritmos - Unidade 2 | ⬜ |  |  |
| 01/06 - 07/06 |  | ⬜ |  |  |
| 08/06 - 14/06 |  | ⬜ |  |  |

**Legenda:** ✅ Estudou | ⬜ Não estudou
